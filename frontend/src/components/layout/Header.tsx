      ...
      <div className="flex items-center gap-2">
        ...
        {projects.length > 0 ? (
          <div className="relative group">
            <button className="flex items-center gap-1.5 text-sm font-semibold text-slate-800 hover:text-brand-600 transition-colors">
              {activeProject?.customer_name ?? 'Выберите проект'}
              <ChevronDown size={14} />
            </button>
            <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-50
                            hidden group-hover:block min-w-[220px] py-1 max-h-72 overflow-y-auto">
              {projects.map((p) => (
                <button
                  key={p.customer_uuid}
